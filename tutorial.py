...
def get_block(size):
    ...
    rect = pygame.Rect(96, 0, size, size) # 96 pixels loads the specific block with grass on it (1st row, 2nd column)
...
class Player(pygame.sprite.Sprite):         # pygame.sprite.Sprite allows for easy collision perfect detection
    ...
    def update_sprite(self):
        ...
        elif self.y_vel > self.GRAVITY * 2:     # Hack to fix the "falling" state, as we programmed the player to align on top of the blocks (decimals are causing the inequality, hence the glitch)
            sprite_sheet = "fall"
        elif self.x_vel != 0:
            sprite_sheet = "run"
...
class Fire(Object):
    ...
    def loop(self):
        ...
        self.mask = pygame.mask.from_surface(self.image) # Allows for pixel perfect collision! If we dont do this, is will be stuck with rectangular hit boxes that are not true to animation

        if self.animation_count // self.ANIMATION_DELAY > len(sprites): # keep the counter small so that it does not take up unnecessary space in memory.
            self.animation_count = 0
...
def handle_vertical_collision(player, objects, dy):
    ...
    for obj in objects:
        if pygame.sprite.collide_mask(player, obj):
            if dy > 0:
                player.rect.bottom = obj.rect.top       # Align the feet of character with top of object
                player.landed()
            elif dy < 0:
...
def main(window):
    ...
    fire = Fire(100, HEIGHT - block_size - 64, 16, 32) # Fire images are 16 by 32
    fire.on()
